...
class BertSeqCls(nn.Module):
    def __init__(self, bert_model_type, num_labels):
        super(BertSeqCls, self).__init__()
        config = AutoConfig.from_pretrained(bert_model_type)
        config.num_labels = num_labels
        self.tokenizer = AutoTokenizer.from_pretrained(bert_model_type)
        self.model = AutoModelForSequenceClassification.from_pretrained(bert_model_type, config=config)

    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        if labels is not None:
            if token_type_ids is not None:
                loss, logits = self.model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, labels=labels)
                return loss, logits
...
def train(model, dataloader, optimizer, scheduler, loss_values, device, log_file, FLAGS):
    ...
    for step, batch in enumerate(dataloader):
        ...
        b_input_ids, b_input_mask, b_input_type, b_labels = batch # DistilBertForSequenceClassification 沒有 token_type_ids
...
def evaluate(model, dataloader, loss_values, device, log_file=None):
    ...
    for batch in dataloader:
        ...
        logits = logits.detach().cpu().numpy().tolist() # [batch_size, 2]
